...
- **Demo downloads:** `public/downloads/*.zip` are generated by `npm run demo-downloads` (see `scripts/generate-demo-downloads.mjs`). Each archive includes guides, checklists, templates, notes, and a small sample PDF.
- **Checkout UX:** Pricing (page + home cards) exposes **checkout** plus **demo ZIP** links; trust copy includes **“Secure digital checkout and automated delivery.”** (and FR/AR equivalents). Order form and thank-you pages add explicit **digital-only / no shipping** lines.
...
- Regenerate demo ZIPs after copy changes: `npm run demo-downloads`
